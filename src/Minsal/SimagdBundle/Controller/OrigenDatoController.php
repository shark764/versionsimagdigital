...
use Minsal\SimagdBundle\Entity\OrigenDatos;
...
class OrigenDatoController extends Controller
{
    ...
    public function probarConexionAction()
    {
        try {
            ...
            if ($this->driver != 'pdo_dblib')
                $conn->connect();
            $mensaje = '<span style="color: green">' . 'Conexión Exitosa' . '</span>';
        ...
        if (preg_match($patron, $sql) == FALSE) {
            try {
                foreach ($conexiones as $cnx) {
                    $datos = array();
                    $cnxObj = $this->getDoctrine()->getManager()->find('MinsalSimagdBundle:Conexion', $cnx);
                    ...
                    $resultado['estado'] = 'ok';
                    $resultado['mensaje'] = '<span style="color: green">' . 'Conexión Exitosa' . '</span>';
    ...
    public function leerOrigenAction(OrigenDatos $origenDato)
    {
        ...
        $campos_existentes = $em->getRepository('minsalCoincidenciasBundle:Campo')->findBy(array('origenDato' => $origenDato));
        ...
        if (count($campos_existentes) == 0 or $recargar == true) {
            ...
            if ($resultado['estado'] == 'ok') {
                ...
                $tipoCampo = $em->getRepository("minsalCoincidenciasBundle:TipoCampo")->findOneByCodigo('integer');
                $util = new \minsal\CoincidenciasBundle\Util\Util();
        ...
        foreach ($campos_existentes as $campo) {
            $campos[$campo->getNombre()]['id'] = $campo->getId();
            $campos[$campo->getNombre()]['significado'] = ($campo->getSignificado()) ? $campo->getSignificado()->getId() : -1;
            $campos[$campo->getNombre()]['significado_codigo'] = ($campo->getSignificado()) ? $campo->getSignificado()->getCodigo() : -1;
            $campos[$campo->getNombre()]['diccionario'] = ($campo->getDiccionario()) ? $campo->getDiccionario()->getId() : -1;
    ...
    public function configurarCampoAction()
    {
        ...
        $valor = $req->get('valor');
        $campo = $em->find("minsalCoincidenciasBundle:Campo", $id);
        $valido = true;
        if ($tipo_cambio == 'tipo_campo') {
            $tipo_campo = $em->find("minsalCoincidenciasBundle:TipoCampo", $valor);
            ...
            if ($valor!=''){
                $significado_variable = $em->find("minsalCoincidenciasBundle:SignificadoCampo", $valor);
                $mensaje = $campo->getNombre() . ': ' . $this->get('translator')->trans('significado_campo_cambiado_a') . ' ' . $significado_variable->getDescripcion();
            }
            else 
            {
                $significado_variable=null;
                $mensaje = $campo->getNombre() . ': ' . $this->get('translator')->trans('significado_campo_cambiado_n');
            }
            
            $campo->setSignificado($significado_variable);
        } else {
            $diccionario = $em->find("minsalCoincidenciasBundle:Diccionario", $valor);
