...
function procedimientoRealizado_actionFormatter(value, row, index) {
	...
	'<div class="btn-toolbar" role="toolbar" aria-label="...">',
		...
		'<a class="show-procedimiento-realizado-action btn btn-primary-v2 btn-xs" href="javascript:void(0)" title="Mostrar examen detallado"' + (row.allowShow === false ? ' disabled="disabled"' : '') + '>',
		    '<i class="glyphicon glyphicon-info-sign"></i>',
		'</a>',
	    '</div>',
	    '<div class="btn-group" role="group">',
		'<a class="edit-procedimiento-realizado-action btn btn-primary-v2 btn-xs " href="' + row.prz_editUrl + '" target="_blank" title="Editar registro de examen"' + (row.allowEdit === false ? ' disabled="disabled"' : '') + '>',
