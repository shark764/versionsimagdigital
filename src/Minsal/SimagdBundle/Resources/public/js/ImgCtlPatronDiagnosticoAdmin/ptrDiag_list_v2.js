...
function actionFormatter(value, row, index) {
	...
	'<div class="btn-toolbar" role="toolbar" aria-label="...">',
		...
		'<a   class="show-patron-diagnostico-action btn btn-primary-v2 btn-outline btn-xs" href="javascript:void(0)" title="Mostrar patrón de diagnóstico detallado"' + (row.allowShow === false ? ' disabled="disabled"' : '') + '>',
		    '<i class="glyphicon glyphicon-info-sign"></i>',
		'</a>',
	    '</div>',
	    '<div class="btn-group" role="group">',
		'<a   class="edit-patron-diagnostico-action btn btn-primary-v2 btn-outline btn-xs " href="javascript:void(0)" title="Editar registro de patrón de diagnóstico"' + (row.allowEdit === false ? ' disabled="disabled"' : '') + '>',
...
window.actionEvents = {
    ...
    'click .edit-patron-diagnostico-action': function (e, value, row, index) {
        ...
        jQuery('#formPtrDiagLabel').removeClass('label-primary-v2')
