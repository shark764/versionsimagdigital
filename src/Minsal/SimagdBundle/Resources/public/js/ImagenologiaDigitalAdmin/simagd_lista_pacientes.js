...
function actionFormatter(value, row, index) {
	...
	'<div class="btn-toolbar" role="toolbar" aria-label="...">',
		...
		'<a class="mostrar-lista-pacientes-action btn btn-primary-v2 btn-xs" href="javascript:void(0)" title="Ver información"' + (row.allowShow === false ? ' disabled="disabled"' : '') + '>',
		    '<i class="glyphicon glyphicon-zoom-in"></i>',
		'</a>',
	    '</div>',
	    '<div class="btn-group" role="group">',
		'<a class="preinscribir-lista-pacientes-action btn btn-element-v2 btn-xs " href="' + row.prc_createUrl + '" target="_blank" title="Solicitar estudio a servicio de Imagenología"' + (row.allowPreinscribir === false ? ' disabled="disabled"' : '') + '>',
