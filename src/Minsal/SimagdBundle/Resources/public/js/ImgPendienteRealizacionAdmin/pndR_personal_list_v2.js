...
function pendienteRealizar_personal_actionFormatter(value, row, index) {
	...
	'<div class="btn-toolbar" role="toolbar" aria-label="...">',
		...
		'<a class="realizar-personal-pendiente-realizar-action btn btn-primary-v2 btn-xs" href="' + row.prz_editUrl + '" target="_blank" title="Realizar examen"' + (row.allowRealizar === false ? ' disabled="disabled"' : '') + '>',
		    '<i class="glyphicon glyphicon-edit"></i>',
		'</a>',
	    '</div>',
	    '<div class="btn-group" role="group">',
		'<a class="almacenar-personal-pendiente-realizar-action btn btn-element-v2 btn-xs " href="javascript:void(0)" title="Actualizar estudio a almacenado en PACS"' + (row.allowActualizarAlmacenado === false ? ' disabled="disabled"' : '') + '>',
