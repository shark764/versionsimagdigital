...
function actionFormatter(value, row, index) {
	...
	'<div class="btn-toolbar" role="toolbar" aria-label="...">',
		...
		'<a   class="show-pacs-establecimiento-action btn btn-primary-v2 btn-outline btn-xs" href="javascript:void(0)" title="Mostrar información de pacs detallada"' + (row.allowShow === false ? ' disabled="disabled"' : '') + '>',
		    '<i class="glyphicon glyphicon-info-sign"></i>',
		'</a>',
	    '</div>',
	    '<div class="btn-group" role="group">',
		'<a   class="edit-pacs-establecimiento-action btn btn-primary-v2 btn-outline btn-xs " href="' + row.pacs_editUrl + '" target="_blank" title="Editar registro de pacs"' + (row.allowEdit === false ? ' disabled="disabled"' : '') + '>',
...
function habilitadoFormatter(value, row, index) {
    return [
        '<span class=\'label label-' + (row.pacs_habilitado === false ? 'danger' : 'primary-v2') + '\'>',
