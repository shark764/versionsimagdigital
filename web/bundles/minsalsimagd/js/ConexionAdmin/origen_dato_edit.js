    ...
    function cargarDatos(recargar_origen) {
        ...
        function(resp) {
            ...
            if (resp.estado === 'error') {
                ...
                var significado_datos= '<OPTION ></OPTION><OPTION value=-1>' + trans.elija_significado_dato + '</OPTION>';
                ...
                $('SELECT.significado').on('select2-clearing', function(e) {
                    //alert($(this).val());
                    $('SELECT.significado').not(this)
                                .children('option[value=' + $(this).val() + ']')
                                .attr('disabled', false)
                    guardar_cambio2($(this));
                });
                ...
                $('SELECT.significado').select2({ placeholder: 'Elija el significado del dato', allowClear: true, width: 'resolve' });
                $('SELECT.tipo_campo').select2({ width: 'resolve' });
                $('SELECT.diccionario').select2({ width: 'resolve' });
                //initSelect2($('SELECT.significado'),false,false,'Seleccione el significado',false);
            }
        });
    }
