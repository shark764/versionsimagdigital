...
function actionCitaFormatter(value, row, index) {
	...
	'<div class="btn-toolbar" role="toolbar" aria-label="...">',
		...
		'<a class="show-cita-action btn btn-primary-v2 btn-xs" href="javascript:void(0)" title="Mostrar cita detallada"' + (row.allowShow === false ? ' disabled="disabled"' : '') + '>',
		    '<i class="glyphicon glyphicon-info-sign"></i>',
		'</a>',
	    '</div>',
	    '<div class="btn-group" role="group">',
		'<a class="edit-cita-action btn btn-primary-v2 btn-xs " href="javascript:void(0)" target="_blank" title="Editar registro de cita"' + (row.allowEdit === false ? ' disabled="disabled"' : '') + '>',
		    '<i class="glyphicon glyphicon-repeat"></i>',
		'</a>',
	    '</div>',
	    '<div class="btn-group" role="group">',
		'<a class="confirm-cita-action btn btn-element-v2 btn-xs " href="javascript:void(0)" target="_blank" title="Confirmar cita"' + (row.allowConfirm === false ? ' disabled="disabled"' : '') + '>',
		    '<i class="glyphicon glyphicon-ok"></i>',
		'</a>',
	    '</div>',
	    '<div class="btn-group" role="group">',
		'<a class="radx-ind-solicitud-cita-action btn btn-primary-v2 btn-xs " href="javascript:void(0)" title="Agregar indicaciones del Médico radiólogo"' + (row.allowIndRadx === false ? ' disabled="disabled"' : '') + '>',
		    '<i class="glyphicon glyphicon-log-in"></i>',
		'</a>',
	    '</div>',
	    '<div class="btn-group" role="group">',
		'<a class="cancel-cita-action btn btn-danger btn-xs " href="javascript:void(0)" target="_blank" title="Cancelar cita"' + (row.allowCancel === false ? ' disabled="disabled"' : '') + '>',
